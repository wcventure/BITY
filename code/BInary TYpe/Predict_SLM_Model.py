...
for i in os.walk(path):
    if i[0]==path:
        ...
        for home, dirs, files in os.walk(i[0]):
            ...
            File_To_Sequence = OnlyFileToSequence('SaveData/Type_List.txt', 'SaveData/Instructions_List.txt')
            #空文件夹的提示
            if files==[]:
            ...
            for filename in files:
                if filename == 'function_prototype':
                    File_To_Sequence.addFunctionPrototype(home + '/' + filename)
            ...
            for filename in files:
                ...
                if re.search('.+\.[tT][xX][tT]$', filename):
                    if filename!=typefile:
                        ...
                        File_To_Sequence.File2Sequence(fileFullName)
            ...
            #for each in data:
                #print(each)
            #print(File_To_Sequence.pointer_sequence_list)
            ...
            #############################################定义一个预测函数，用于迭代
            def Predict(File_To_Sequence,data,printre=0):
                ...
                if not File_To_Sequence.pointer_label_list==[]:
                    AreThereAnyPointer=1

                    pointerdata = copy.deepcopy(File_To_Sequence.pointer_sequence_list)

                    for each in pointerdata:  # 插入表头
                        each.insert(0, 'start')

                    predictlabel_list_pointer = [0]
                    predictlabel_list_pointer = predictlabel_list_pointer * len(pointerdata)

                    # SLM模型预测
                    for k in range(0, len(pointerdata)):
                        mostIndex = 999
                        biggest = 0
                        for i in range(0, len(ProbabilityTable)):
                            if ComputingProbability(data[k], ProbabilityTable[i]) > biggest:
                ...
                for each_var, each_predictlabel,each_offset in zip(File_To_Sequence.Name_list,predictlabel_list,File_To_Sequence.Offset_list):
                    if each_predictlabel==999:
                        continue
                ...
                if AreThereAnyPointer==1:
                    for each_var, each_predictlabel in zip(File_To_Sequence.pointer_name_list,predictlabel_list_pointer):
                        if each_predictlabel == 999:
                            continue
                ...
                if varPointerResult != [] or File_To_Sequence.FP_Pointer_rel!=[]:#此处确定过才改的 or, 后面if语句那个是and
                    #print('Pointer Detail:')
                    FPset=set(File_To_Sequence.FP_Pointer_rel)#集合去重复
                    for each in FPset:
                ...
                if File_To_Sequence.UDrelation != [] and varPointerResult != []:
                    #print('Same pointerType:')#后来发现不行了，有可能这两个列表都不空，但是没有交集
                    for each in File_To_Sequence.UDrelation:
                        left, right = each.split('---')
                ...
                # 添加指向关系
                #print(File_To_Sequence.LEArelation)
                if not File_To_Sequence.LEArelation == []:
                    for eachRelation in File_To_Sequence.LEArelation:
                ...
                File_To_Sequence.PredictResult = []
                File_To_Sequence.PointerResult = []
                File_To_Sequence.PredictResult = copy.deepcopy(varPredictResult)
                File_To_Sequence.PointerResult = copy.deepcopy(varPointerResult)
            #######################################以上为predict函数定义

            #预测完后，有新的信息要整合
            def flashNewInfo(File_To_Sequence , PredictResult):
                newFPList = []
                for eachFAP in File_To_Sequence.FuncAndPara:
                ...
                for eachC in File_To_Sequence.CallParaSet:
                    left, right = eachC.split('---', 1)
                    for each in newFPList:
                        if right == each['funcPara'] or right == '_' + each['funcPara']:
                            #print(left,each['paraType'])
                            if '*' in each['paraType']:
                                #File_To_Sequence.updateSequence(left, 'FPptr', 0.45)
                                if '*' == each['paraType']:
                                    break
                            #File_To_Sequence.updateSequence(left, 'FP' + re.sub('[^a-zA-Z]', '', each['paraType']), 0.08)
                ...
                for each in PredictResult:
                    if each['varType'] == '*':
                        for sequence,name in zip(File_To_Sequence.Sequence_list,File_To_Sequence.Name_list):
                            if name == each['varName']:
                                saveweight = 0
                                saveT = ''
                                for eachweight,eachinstruction in zip(sequence,File_To_Sequence.Instructions_list):
                                    if 'FP' in eachinstruction and eachinstruction!='FPptr':
            ...
            # 输出函数
            def printPredict(PredictResult,solve=0):
                print('**********************************************************************')
                print(home, end=':\n')
                print('')
                for each in PredictResult:
                ...
                if File_To_Sequence != []:
                    print('\nPoint to analysis:')
                    for each in File_To_Sequence.LEArelation:
                        print('\t' + each)
            ...
            # -----------------------------------------------------
            # 首次预测
            data = copy.deepcopy(File_To_Sequence.Sequence_list)
            label = copy.deepcopy(File_To_Sequence.Label_list)
                        # Z_ScoreNormalization(data,5)#向量-1~1归一化处理
            # MaxMinNormalization(data,0)#向量zscore归一化处理

            Predict(File_To_Sequence, data)
            flashNewInfo(File_To_Sequence , File_To_Sequence.PredictResult)
            File_To_Sequence.UpdateUDrelation()
            PredictResult = File_To_Sequence.PredictResult

            #printPredict(PredictResult)
            # -----------------------------------------------------
            ...
            Predict(File_To_Sequence, data, 1)
            flashNewInfo(File_To_Sequence, File_To_Sequence.PredictResult)
            File_To_Sequence.UpdateUDrelation()
            PredictResult = File_To_Sequence.PredictResult

            #printPredict(PredictResult, 1)
