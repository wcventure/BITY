...
def SequenceReplaceSequence(a,b):
    b=[]
    b.extend(a)
                ...
                for i in range(0,len(self.Name_list)):
                    for j in range(0,len(self.Name_list)):
                        if left==self.Name_list[i] and right==self.Name_list[j]:
                            ...
                            s=j
                            break
                SequenceAddSequence(self.Sequence_list[f],self.Sequence_list[s])
                ...
                for i in range(0,len(self.Name_list)):
                    for j in range(0,len(self.Name_list)):
                        if left==self.Name_list[i] and right==self.Name_list[j]:
                            ...
                            s=j
                            break
                SequenceReplaceSequence(self.Sequence_list[s],self.Sequence_list[f])


    #函数File2Sequence将一个汇编代码文件中的每个变量用一个向量的形式表示
    #该函数的第一个参数为汇编代码文件的地址,第二个参数为从源代码提取的类型信息的文件
    def File2Sequence(self,filename,type_file):
        ...
        try:#打开汇编代码文件
            with open(filename,encoding='utf-8') as assemblyfile:
                ...
                for each_var in variables:
                    Sequence_list_element=[]#Sequence_list的组成元素,初始化为全0

                    #定义一个下面经常用的函数
                    #将元素添加到序列
                    def updataSequence(IR):
                        for each_instruction in self.Instructions_list:
                            if each_instruction == IR:
                                Sequence_list_element.append(IR)
                                break
                        ...
                        if re.search(regex,fileContext[lineNum]):
                                    ...
                                    if 'qword ptr' in fileContext[lineNum] or 'QWORD PTR' in fileContext[lineNum]:
                                        ...
                                        for each_instruction in self.Instructions_list:
                                            if each_instruction == 'qword':
                                                Sequence_list_element.insert(0,'qword')
                                                break
                                            count += 1
                                        ...
                                        for each_instruction in self.Instructions_list:
                                            if each_instruction == 'dword':
                                                Sequence_list_element.insert(0, 'dword')
                                                break
                                            count += 1
                                        ...
                                        for each_instruction in self.Instructions_list:
                                            if each_instruction == 'word':
                                                Sequence_list_element.insert(0, 'word')
                                                break
                                            count += 1
                                        ...
                                        for each_instruction in self.Instructions_list:
                                            if each_instruction == 'byte':
                                                Sequence_list_element.insert(0, 'byte')
                                                break
                                            count += 1
                                            ...
                                            if funcPara == each['funcPara'] or funcPara == '_' + each['funcPara']:
                                                if '*' in each['paraType']:
                                                    if each_var['byteNum'] == 'dword':
                                                        self.FPinfo.append('FPptr')
                                                        if '*' == each['paraType']:
                                                            break
                                                        self.FPinfo.append('FP' + re.sub('[^_a-zA-Z0-9]','',each['paraType']))
                                                else:
                            ...
                            #有逗号的就是二元指令
                                            ...
                                            try:
                                                for k in range(1, 6):
                                                    ...
                                                    if lineNum - k > 0 and ',' in fileContext[lineNum - k]:
                                                        ...
                                                        updataSequence(registerInstruction)
                                                            ...
                                                            for each in self.function_parameter_list:
                                                                if funcRe == each['funcPara'] or funcRe == '_' + each['funcPara']:
                                                                    if '*' in each['paraType']:
                                                                        ...
                                                                        if '*' == each['paraType']:
                                                                            break
                                                                    self.FPinfo.append('FP' + re.sub('[^a-zA-Z]', '',each['paraType']))
                                                                    break
                                ...
                                #如果下划线在右边
                                    ...
                                    print("File2Sequence Error:cannot find '_'")

                            ##print(instructionIR)

                            updataSequence(instructionIR)

                    #dword,word,byte这些信息直接给一个较大的值，比如75
                    if issearchwordptr == 0:
                        count=0
                        for each_instruction in self.Instructions_list:
                            if Sequence_list_element==[]:
                                break
                            if each_instruction==each_var['byteNum']:
                                if 'word' not in Sequence_list_element[0] and 'byte' not in Sequence_list_element[0]:
                                    Sequence_list_element.insert(0,each_var['byteNum'])
                                    break
                            count+=1

                    #下面这个for语句中的代码把相应的标签增加到self.Label_list中，同时把前面的结果追加到self.Sequence_list中
                                ...
                                if re.match(regex, each):
                                    ...
                                    for ea in self.Type_list:
                                        if ea==typeDeclar:
                                            #如果向量为0向量，那么去掉该向量
                                            flag=0
                                            if len(Sequence_list_element) > 2:
                                                flag = 1

                                            #不是0向量的是有效结果
                                            if flag==1:
                                                #print(Sequence_list_element)
                                                #把这个结果追加到self.Sequence_list中
                                                self.Sequence_list.append(Sequence_list_element)
...
class OnlyFileToSequence:
    ...
    # 函数File2Sequence将一个汇编代码文件中的每个变量用一个向量的形式表示
    # 该函数的第一个参数为汇编代码文件的地址
    def File2Sequence(self, filename):
        indirctVar = []
        variables = []  # 用于保存各个变量的信息的列表，列表中的元素是字典
        newPointerList = []  # 新预测的指针
        ...
        try:  # 打开汇编代码文件
            with open(filename,encoding='utf-8') as assemblyfile:
                ...
                for each_var in variables:
                        ...
                        if re.search(regex1, fileContext[lineNum]) or re.search(regex2, fileContext[lineNum]):
                            ...
                            for each_instruction in self.Instructions_list:
                                if each_instruction == 'bias in stackframe':
                                    Sequence_list_element[count] = 25
                                    break
                                count += 1
                                """
                        # 查找[ebp+...+varname+...]的相关语句
                    ...
                    flag = 0
                    CountNotZero = 0
                    for i in range(0,len(Sequence_list_element)):
                        if len(Sequence_list_element) > 0:
                            flag = 1
                            break

                    # 不是0向量的是有效结果
                    #if flag == 1:
                    # 把前面的结果追加到self.Sequence_list中
                    self.Sequence_list.append(Sequence_list_element)
                    # 把相应的函数及其变量名增加到self.Name_list_list中
        ...
        for each in L:
            Sequence_list_element = []  # Sequence_list的组成元素,初始化为全0
                ...
                # 定义一个下面经常用的函数
                # 在指令字典中查找，如果有的话就在相应的向量位置上+1
            def updataSequenceOnlyHead(head):
                # 在指令字典中查找，如果有的话就在相应的向量位置上+1
                count = 0
                for each_instruction in self.Instructions_list:
                    if head in each_instruction:
                        if 'byte' in head or 'word' in head:
                            Sequence_list_element.insert(0,each_instruction)
                            break

                        Sequence_list_element.append(each_instruction)
                        break
                    elif each_instruction == '#':
            ...
            for every in newPointerList:
                if each==every['PointName']:
                    ...
                    if 'imm' in instructionNear:
                        updataSequenceOnlyHead(head + ' _ ' + 'imm')
            ...
            for e in FPset:
                ...
                if functionName + ':' + each == left:
                    if '*' in right:
                        ...
                        if "*" == right:
                            break
                    self.FPinfo.append('FP' + right.replace('*',''))
                    self.FPinfo.append('FP' + right.replace('*',''))
                    self.FPinfo.append('FP' + right.replace('*',''))
            ...
            self.pointer_sequence_list.append(Sequence_list_element)
