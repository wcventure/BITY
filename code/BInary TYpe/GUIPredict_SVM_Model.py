...
#导入测试数据集
#path = 'Microsoft Visual Studio 11.0-VC-crt-src'
#path = 'TestData_x86'
...
for i in os.walk(path):
    if i[0]==path:
        ...
        for home, dirs, files in os.walk(i[0]):
            ...
            for filename in files:
                ...
                if re.search('.+\.[tT][xX][tT]$', filename):
                    if filename!=typefile:
                        ...
                        with open(fileFullName) as whatmode:
                            for i in range(3,20):
            ...
            structTypeOutput = []
            #############################################定义一个预测函数，用于迭代
            def Predict(File_To_Vector,data,label,model,printre=0):
            ...
            def outputFile(FileName, globalPredictResult, solve=0):
                outputList = []
                outputList.append('**********************************************************************\n')
            ...
            pointerdata = copy.deepcopy(File_To_Vector.pointer_vector_list)
            ...
            Predict(File_To_Vector, data, label, model)
            ...
            Predict(File_To_Vector, data, label, model,1)
            flashNewInfo(File_To_Vector, File_To_Vector.PredictResult)
            File_To_Vector.UpdateUDrelation()
            PredictResult = File_To_Vector.PredictResult
            ...
            for each1, each2, each3 in zip(File_To_Vector.Name_list, File_To_Vector.Vector_list , data):
                print(each1)
                for i,j,k in zip(File_To_Vector.Instructions_list,each2,each3):
                    if j!=0:
                        print('\t'+i+'\t\t'+str(j) + '\t' +str(k))

            for each1, each2, each3 in zip(File_To_Vector.globalVarName, File_To_Vector.globalVarVector , globaldata):
                print(each1)
                for i,j,k in zip(File_To_Vector.Instructions_list,each2,each3):
                    if j!=0:
                        print('\t'+i+'\t\t'+str(j) + '\t' +str(k))

            for each1, each2, each3 in zip(File_To_Vector.pointer_name_list, pointerdata , File_To_Vector.pointer_vector_list):
                print(each1,end=' ptr->\n')
                for i, j ,k in zip(File_To_Vector.Instructions_list, each2 ,each3):
                    if j != 0:
                        print('\t' + i + '\t\t' + str(j) + '\t' +str(k))
